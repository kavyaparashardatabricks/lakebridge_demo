...
CREATE OR REPLACE PROCEDURE analytics.sp_refresh_customer_360 (
    in_run_date     DATE,
    in_batch_size   INTEGER DEFAULT 50000,
    out_rows_merged OUT BIGINT,
    out_status      OUT VARCHAR(40)
)
LANGUAGE plpgsql
AS $$
DECLARE
    v_sql           VARCHAR(MAX);
    v_partition     VARCHAR(40);
    v_lower         TIMESTAMP;
    v_upper         TIMESTAMP;
    v_part_cur      REFCURSOR;
    v_rowcount      BIGINT := 0;
    v_total         BIGINT := 0;
    v_start         TIMESTAMP := GETDATE();
BEGIN
    -- Audit start
    ...
    v_lower := in_run_date::TIMESTAMP;
    v_upper := DATEADD(day, 1, in_run_date)::TIMESTAMP;
    ...
    LOOP
        ...
        v_sql := 'INSERT INTO tmp_c360_stage '
              ...
              || '       JSON_PARSE(''{"country":"'' || $1 || ''","tier":"gold"}''), '
    ...
    WHEN NOT MATCHED THEN INSERT (
        ...
        src.unique_sessions, src.hll_sessions, GETDATE()
    );
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        VALUES ('sp_refresh_customer_360', in_run_date, SQLERRM, GETDATE());
        RAISE;
END;
$$;
...
JOIN    shared_marketing.public.campaign_attribution        m
  USING (customer_id)
WHERE   c.snapshot_date = CURRENT_DATE - 1
  AND   m.last_touch_ts >= DATEADD(day, -7, GETDATE());
...
JOIN    sales.customers     c  USING (customer_id)
JOIN    catalog.products    p  USING (sku)
WHERE   o.order_ts >= DATEADD(year, -1, GETDATE())
